...
app.post('/mcp', async (request, reply) => { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST', details: parsed.error.flatten() }); const body: any = parsed.data; const result = body.method ? await handleMcpMessage(firewall, body, { agent: request.headers['mcp-agent'], server: request.headers['mcp-server'] }) : await firewall.handleCall(body); if (database && result?.audit) { try { await syncConfiguration(database, firewall); await appendAudit(database, result.audit); } catch (error) { request.log.error({ error }, 'postgres synchronization failed'); } } return reply.header('MCP-Protocol-Version', '2025-06-18').send(body.method ? { jsonrpc: '2.0', id: body.id, result: result.error ? undefined : result, error: result.error || undefined } : result); });
