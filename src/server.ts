...
app.setNotFoundHandler(async (request, reply) => { if (request.method === 'GET' && !request.url.startsWith('/api/') && !request.url.startsWith('/mcp')) return reply.sendFile('index.html'); return reply.code(404).send({ error: 'NOT_FOUND' }); });

app.get('/health', async () => ({ status: 'ok', database: Boolean(database), mcp: Boolean(mcpServer) }));
app.post('/mcp', async (request, reply) => tracer.startActiveSpan('mcp.request', async (span) => { try { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST', details: parsed.error.flatten() }); const body: any = parsed.data; span.setAttribute('mcp.method', body.method || 'tools/call'); const result = body.method ? await handleMcpMessage(firewall, body, { agent: request.headers['mcp-agent'], server: request.headers['mcp-server'] }) : await firewall.handleCall(body); if (database && result?.audit) { try { await syncConfiguration(database, firewall); await appendAudit(database, result.audit); } catch (error) { request.log.error({ error }, 'postgres synchronization failed'); } } return reply.header('MCP-Protocol-Version', '2025-06-18').send(body.method ? { jsonrpc: '2.0', id: body.id, result: result.error ? undefined : result, error: result.error || undefined } : result); } finally { span.end(); } }));
app.post('/mcp/tools/call', async (request, reply) => tracer.startActiveSpan('mcp.compatibility_request', async (span) => { try { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST' }); const result = await firewall.handleCall(parsed.data as any); if (database && result.audit) { try { await syncConfiguration(database, firewall); await appendAudit(database, result.audit); } catch (error) { request.log.error({ error }, 'postgres synchronization failed'); } } const status = result.error?.code === 'TARGET_ERROR' ? 502 : result.decision === 'ALLOW' ? 200 : result.decision === 'REQUIRE_APPROVAL' ? 202 : 403; return reply.code(status).send(result); } finally { span.end(); } }));
