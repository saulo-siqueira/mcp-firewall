...
app.post('/mcp/tools/call', async (request, reply) => { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST' }); const result = await firewall.handleCall(parsed.data as any); return reply.code(result.decision === 'ALLOW' ? 200 : result.decision === 'REQUIRE_APPROVAL' ? 202 : 403).send(result); });
