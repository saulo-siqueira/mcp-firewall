...
app.post('/mcp/tools/call', async (request, reply) => tracer.startActiveSpan('mcp.compatibility_request', async (span) => { try { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST' }); const result = await firewall.handleCall(parsed.data as any); if (database && result.audit) { try { await syncConfiguration(database, firewall); await appendAudit(database, result.audit); } catch (error) { request.log.error({ error }, 'postgres synchronization failed'); } } return reply.code(result.decision === 'ALLOW' ? 200 : result.decision === 'REQUIRE_APPROVAL' ? 202 : 403).send(result); } finally { span.end(); } }));
