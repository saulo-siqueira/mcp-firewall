...
app.all('/mcp/sdk', async (request, reply) => { const requested = request.headers['mcp-session-id'] as string | undefined; let connection = requested ? transports.get(requested) : undefined; if (!connection) { const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: () => crypto.randomUUID() }); const server = createSdkServer(); await server.connect(transport); connection = { transport, server }; if (request.method === 'POST' && !(request.body as any)?.method?.startsWith('notifications/')) transports.set(transport.sessionId!, connection); transport.onclose = () => { if (transport.sessionId) transports.delete(transport.sessionId); }; } reply.hijack(); await connection.transport.handleRequest(request.raw, reply.raw, request.method === 'GET' ? undefined : request.body); });
...
const adminApi = async (request: any, reply: any) => {
  ...
  if (database && result.status < 400) { try { await syncConfiguration(database, firewall); } catch (error) { request.log.error({ error }, 'postgres synchronization failed'); } }
