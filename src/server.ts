...
const callSchema = z.object({ jsonrpc: z.string().optional(), id: z.union([z.string(), z.number(), z.null()]).optional(), agent: z.string().optional(), server: z.string().optional(), tool: z.string().optional(), arguments: z.record(z.string(), z.unknown()).optional(), method: z.string().optional(), params: z.record(z.string(), z.unknown()).optional() });
...
app.post('/mcp', async (request, reply) => tracer.startActiveSpan('mcp.request', async (span) => { try { const parsed = callSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: 'INVALID_MCP_REQUEST', details: parsed.error.flatten() }); const body: any = parsed.data; span.setAttribute('mcp.method', body.method || 'tools/call'); const result: any = body.method ? await handleMcpMessage(firewall, body, { agent: request.headers['mcp-agent'], server: request.headers['mcp-server'] }) : await firewall.handleCall(body); if (database && result?.audit) { try { await syncConfiguration(database, firewall); await appendAudit(database, result.audit); } catch (error) { request.log.error({ error }, 'PostgreSQL synchronization failed'); } } if (!body.method) return reply.header('MCP-Protocol-Version', '2025-06-18').code(result.error?.code === 'TARGET_ERROR' ? 502 : result.decision === 'ALLOW' ? 200 : result.error?.code === 'APPROVAL_UNAVAILABLE' ? 409 : result.decision === 'REQUIRE_APPROVAL' ? 202 : 403).send(result); const envelope = result.error ? { jsonrpc: '2.0', id: body.id ?? null, error: result.error } : { jsonrpc: '2.0', id: body.id ?? null, result }; const status = result.error?.data?.error?.code === 'TARGET_ERROR' || result.error?.code === 'TARGET_ERROR' ? 502 : result.error ? 403 : 200; return reply.header('MCP-Protocol-Version', '2025-06-18').code(status).send(envelope); } finally { span.end(); } }));
...
app.get('/api/dashboard', async (request, reply) => { const session = sessionFrom(request); const query: any = request.query || {}; const view = query.view || 'dashboard'; if (view !== 'login' && firewall.users.size && !firewall.authorize(session)) return reply.code(401).send({ error: 'UNAUTHORIZED', login: '/api/dashboard?view=login' }); if (String(request.headers.accept || '').includes('application/json') || query.format === 'json') { const result = await firewall.api('GET', '/api/dashboard', {}, session); return reply.code(result.status).send(result.body); } return reply.type('text/html').send(view === 'login' || !firewall.users.size ? firewall.loginHtml() : firewall.renderView(view)); });
const adminApi = async (request: any, reply: any) => {
  ...
  const result = await firewall.api(request.method, request.url, body, sessionId);
  ...
  if (result.body?.session?.id && (request.url.startsWith('/api/auth/login') || request.url.startsWith('/api/auth/setup'))) reply.header('set-cookie', `session=${encodeURIComponent(result.body.session.id)}; HttpOnly; SameSite=Lax; Path=/`);
