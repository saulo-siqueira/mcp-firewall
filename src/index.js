...
const invokeTarget = async (server, call) => {
  ...
  const response = await fetch(server.url, { method: 'POST', headers: { accept: 'application/json, text/event-stream', 'content-type': 'application/json', 'MCP-Protocol-Version': '2025-06-18', ...(server.headers || {}) }, body: JSON.stringify({ jsonrpc: '2.0', id: id('mcp'), method: 'tools/call', params: { name: call.tool, arguments: call.arguments } }) });
  if (!response.ok) throw new Error(`http MCP server returned ${response.status}`); const text = await response.text(); if (response.headers.get('content-type')?.includes('text/event-stream')) { const data = text.split(/\r?\n/).find((line) => line.startsWith('data:')); return JSON.parse(data?.slice(5).trim() || '{}'); } return JSON.parse(text || '{}');
};
