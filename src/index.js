...
const invokeTarget = async (server, call) => {
  ...
  const response = await fetch(server.url, { method: 'POST', headers: { 'content-type': 'application/json', ...(server.headers || {}) }, body: JSON.stringify({ jsonrpc: '2.0', id: id('mcp'), method: 'tools/call', params: { name: call.tool, arguments: call.arguments } }) });
  if (!response.ok) throw new Error(`http MCP server returned ${response.status}`); return response.json();
};
...
export const loadConfigFile = (file, options = {}) => { const config = parseConfigYaml(readFileSync(file, 'utf8')); return new Firewall({ ...options, policies: config.policies, servers: config.mcp_servers, configAuthoritative: true }); };
...
export class Firewall {
  constructor({ policies = [], servers = [], approvalAvailable = true, storagePath = null, configAuthoritative = false } = {}) {
  ...
  serversHtml() { return shell('MCP Servers', `<h1>MCP Servers</h1>${[...this.servers.values()].map((s) => `<article>${s.name}: <strong>${s.status}</strong> · ${s.transport}</article>`).join('') || '<p class="empty">No MCP servers configured.</p>'}<form id="server-create"><input name="name" placeholder="Name" required><select name="transport"><option value="stdio">stdio</option><option value="http">http</option></select><input name="target" placeholder="Command or URL" required><button>Add MCP Server</button></form>`); }
