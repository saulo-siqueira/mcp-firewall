...
export class Firewall {
  ...
  serversHtml() { return shell('MCP Servers', `<h1>MCP Servers</h1>${[...this.servers.values()].map((s) => `<article data-server-name="${escapeHtml(s.name)}">${escapeHtml(s.name)}: <strong>${escapeHtml(s.status)}</strong> · ${s.transport}<button data-delete-server="${escapeHtml(s.name)}">Delete</button></article>`).join('') || '<p class="empty">No MCP servers configured.</p>'}<form id="server-create"><input name="name" placeholder="Name" required><select name="transport"><option value="stdio">stdio</option><option value="http">http</option></select><input name="target" placeholder="Command or URL" required><button>Add MCP Server</button></form>`); }
