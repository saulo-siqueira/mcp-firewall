...
export class Firewall {
  ...
  dashboardHtml() { const allowed = [...this.audit.values()].filter((x) => x.decision === 'ALLOW').length; const blocked = [...this.audit.values()].filter((x) => x.decision === 'DENY').length; const approvals = [...this.audit.values()].filter((x) => x.decision === 'REQUIRE_APPROVAL').length; return page('Dashboard', `<h1>Dashboard</h1><section class="metrics"><b>Total Tool Calls ${this.audit.size}</b><b>Allowed ${allowed}</b><b>Blocked ${blocked}</b><b>Requires Approval ${approvals}</b></section><h2>Recent Calls</h2><h2>Active Policies</h2><h2>Secrets Intercepted</h2>`); }
  auditDetailHtml(auditId) { const event = this.audit.get(auditId); return page('Audit detail', `<h1>Audit detail</h1><pre>${escapeHtml(JSON.stringify(event, null, 2))}</pre>`); }
  approvalsHtml() { return page('Approvals', `<h1>Approvals</h1>${[...this.approvals.values()].map((a) => `<article><strong>${a.status}</strong> ${a.tool}<button>Approve</button><button>Reject</button></article>`).join('')}`); }
  serversHtml() { return page('MCP Servers', `<h1>MCP Servers</h1>${[...this.servers.values()].map((s) => `<article>${s.name}: ${s.status}</article>`).join('')}`); }
}

const escapeHtml = (value) => String(value).replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c]);
const page = (title, body) => `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${title}</title><style>body{background:#fcfaf7;color:#423d38;font:14px system-ui;margin:32px}button{background:#fe6e00;color:white;border:0;padding:8px 12px;margin:8px;border-radius:6px}.metrics{display:flex;gap:16px}.metrics b{padding:24px;background:#fff;border-radius:12px}</style></head><body>${body}</body></html>`;
