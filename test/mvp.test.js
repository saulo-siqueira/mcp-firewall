...
test('cli_headless_starts_real_stdio_gateway_without_dashboard', async () => { const { mkdtemp } = await import('node:fs/promises'); const { tmpdir } = await import('node:os'); const { join } = await import('node:path'); const dir = await mkdtemp(join(tmpdir(), 'mcp-fw-cli-')); const child = spawn(process.execPath, [join(process.cwd(), 'src/cli.js'), 'start', '--headless'], { cwd: dir, stdio: ['pipe', 'pipe', 'pipe'] }); let output = ''; child.stdout.on('data', (chunk) => { output += chunk; if (output.includes('listening on STDIO')) child.stdin.write('{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}\n'); }); const status = await new Promise((resolve, reject) => { const timer = setTimeout(() => { child.kill(); reject(new Error('headless gateway did not respond')); }, 3000); child.stdout.on('data', (chunk) => { if (String(chunk).includes('"id":1')) { clearTimeout(timer); child.kill(); resolve(0); } }); child.on('error', reject); }); assert.equal(status, 0); assert.match(output, /Headless mode: enabled/); assert.match(output, /"protocolVersion":"2025-06-18"/); assert.doesNotMatch(output, /Gateway listening on port/); });
