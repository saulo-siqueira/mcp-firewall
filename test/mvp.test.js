...
test('admin_screens_render_live_collections', async () => { const firewall = new Firewall({ policies: [{ name: 'read', match: { tool: 'x' }, action: 'allow', enabled: true }], servers: [{ name: 'db', transport: 'http', status: 'Connected' }] }); await firewall.handleCall(context()); assert.match(firewall.toolCallsHtml(), /filesystem\.read/); assert.match(firewall.policiesHtml(), /read/); assert.match(firewall.serversHtml(), /Connected/); assert.match(firewall.approvalsHtml(), /No pending approvals/); });
