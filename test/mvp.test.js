...
test('redaction_covers_secrets returned by an MCP server and audit result', async () => { const firewall = new Firewall({ servers: [{ name: 's', handler: async () => ({ apiKey: 'returned-secret', nested: 'TOKEN=returned-secret' }) }], policies: [{ match: { tool: 'x' }, action: 'allow', enabled: true }] }); const result = await firewall.handleCall({ agent: 'a', server: 's', tool: 'x', arguments: {} }); assert.equal(result.result.apiKey, '[REDACTED]'); assert.equal(result.audit.result.apiKey, '[REDACTED]'); assert.ok(!JSON.stringify(result.audit).includes('returned-secret')); });
...
test('audit_detail_renders_redacted_call_and_approval_data', async () => { const firewall = new Firewall({ servers: [{ name: 'filesystem', handler: async () => ({ ok: true }) }], policies: [{ match: { tool: 'filesystem.read' }, action: 'require_approval', enabled: true }] }); const result = await firewall.handleCall(context({ arguments: { password: 'secret' } })); await firewall.approve(result.approval.id, 'admin'); const html = firewall.auditDetailHtml(result.audit.id); for (const value of ['[REDACTED]', 'filesystem.read', 'REQUIRE_APPROVAL', 'admin', 'approved_at', 'duration', 'policy']) assert.match(html, new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))); });
...
test('admin_api_supports_dashboard_metrics_filters_and_audit_detail', async () => { const firewall = new Firewall({ servers: [{ name: 'filesystem', handler: async () => ({ ok: true }) }], policies: [{ name: 'read', match: { tool: 'filesystem.read' }, action: 'allow', enabled: true }] }); const setup = firewall.api('POST', '/api/auth/setup', { name: 'Admin', email: 'a@example.com', password: 'pw', confirmPassword: 'pw' }); const session = setup.body.session.id; const call = await firewall.handleCall(context()); const summary = firewall.api('GET', '/api/dashboard?format=json', {}, session); assert.equal(summary.status, 200); assert.equal(summary.body.total_tool_calls, 1); assert.equal(firewall.api('GET', '/api/tool-calls?tool=filesystem.read', {}, session).body.length, 1); assert.equal(firewall.api('GET', `/api/audit-logs/${call.audit.id}`, {}, session).body.id, call.audit.id); assert.equal(firewall.api('GET', '/api/audit-logs/missing', {}, session).status, 404); });
test('restore_rejects_audit_decisions_outside_the_MVP_enum', async () => { assert.throws(() => new Firewall().restore({ audit: [{ id: 'bad', decision: 'APPROVED' }] }), /invalid audit decision/); });
test('legacy_api_server_preserves_collection_query_filters', async () => { const firewall = new Firewall(); firewall.setupAdmin({ name: 'Admin', email: 'legacy@example.com', password: 'pw', confirmPassword: 'pw' }); const session = firewall.login('legacy@example.com', 'pw').session.id; firewall.audit.set('one', { id: 'one', agent: 'a', server: 's', tool: 'one', decision: 'ALLOW', arguments: {}, duration: 1, result: null, created_at: new Date().toISOString() }); firewall.audit.set('two', { id: 'two', agent: 'a', server: 's', tool: 'two', decision: 'DENY', arguments: {}, duration: 1, result: null, created_at: new Date().toISOString() }); const server = createApiServer(firewall); await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve)); const address = server.address(); const response = await fetch(`http://127.0.0.1:${address.port}/api/tool-calls?tool=one`, { headers: { 'x-session-id': session } }); assert.equal(response.status, 200); assert.deepEqual((await response.json()).map((event) => event.id), ['one']); await new Promise((resolve) => server.close(resolve)); });
