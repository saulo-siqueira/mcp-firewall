...
test('persistent_firewall_restores_policies_users_and_audit', async () => { const { mkdtemp } = await import('node:fs/promises'); const { tmpdir } = await import('node:os'); const { join } = await import('node:path'); const { readFile } = await import('node:fs/promises'); const dir = await mkdtemp(join(tmpdir(), 'mcp-fw-store-')); const storagePath = join(dir, 'state.json'); const first = new Firewall({ storagePath, servers: [{ name: 's', handler: async () => ({ ok: true }) }], policies: [{ name: 'p', match: { tool: 'x' }, action: 'allow', enabled: true }] }); first.setupAdmin({ name: 'Admin', email: 'a@example.com', password: 'pw', confirmPassword: 'pw' }); await first.handleCall({ agent: 'a', server: 's', tool: 'x', arguments: {} }); const restored = new Firewall({ storagePath }); assert.ok(restored.getPolicy('p')); assert.equal(restored.users.size, 1); assert.equal(restored.audit.size, 1); assert.match(await readFile(storagePath, 'utf8'), /created_at/); });
...
test('cli_headless_starts_without_dashboard', async () => { const firewall = new Firewall(); assert.equal(firewall.start({ headless: true }).headless, true); const result = spawnSync(process.execPath, ['src/cli.js', 'start', '--headless', '--check'], { encoding: 'utf8' }); assert.equal(result.status, 0); assert.match(result.stdout, /Headless mode: enabled/); assert.doesNotMatch(result.stdout, /Dashboard: http/); });
