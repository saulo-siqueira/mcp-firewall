...
test('dashboard_renders_required_metrics_and_sections', async () => { const firewall = new Firewall(); const html = firewall.dashboardHtml(); for (const label of ['Total Tool Calls', 'Allowed', 'Blocked', 'Requires Approval', 'Recent Calls', 'Active Policies', 'Secrets Intercepted']) assert.match(html, new RegExp(label)); });
